...
class RandomSurvivalForest():
    ...
    def __init__(self, n_trees=10, n_trees_in_subset=2, max_features=2, max_depth=5, min_samples_split=2,
                 ...
                 random_seed=None, split_type="best"):
        """
...
#change here
        ...
        if highest_ig == 0:
            ...
            return feature_split_pairs[information_gains.index(highest_ig)-1]
    #my function
    def statlogrank(self,N,nb_inf,nb_sup,sum1,sum2, get_time):
            """
                Perform logrank test.
                :param DataFrame x: Samples data frame.
                :param str feature: Column name.
            ...
            for i in range(N):  
                t_i=get_time[i]
                #left child
                if t_i in sum1.index:
                    #print ("left,", t_i)
                    count_inf[i]=sum1.loc[t_i,0]+sum1.loc[t_i,1]
                    d[1,i]=sum1.loc[t_i,1]
                if  not (t_i in sum1.index):
                    count_inf[i]=0
                    d[1,i]=0
               #right child     
                if t_i in sum2.index:
                    #print ("right,", t_i)
                    count_sup[i]=sum2.loc[t_i,0]+sum2.loc[t_i,1]
                    d[2,i]=sum2.loc[t_i,1]
                if not(t_i in sum2.index):
                    count_sup[i]=0
                    d[2,i]=0
