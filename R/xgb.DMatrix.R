...
xgb.get.DMatrix <- function(data, label = NULL, missing = NA, weight = NULL,censor=NULL) {
...
getinfo.xgb.DMatrix <- function(object, name, ...) {
  if (typeof(name) != "character" ||
      length(name) != 1 ||
      !name %in% c('label', 'weight', 'base_margin', 'nrow','censor')) {
    stop("getinfo: name must be one of the following\n",
         "    'label', 'weight', 'base_margin', 'nrow','censor'")
  }
  if (name != "nrow"){
...
setinfo.xgb.DMatrix <- function(object, name, info, ...) {
  ...
  if (name == "censor") {
    #change
    if (length(info) != nrow(object))
      stop("The length of censors must equal to the number of rows in the input data")
    .Call(XGDMatrixSetInfo_R, object, name, as.numeric(info))
    return(TRUE)
  }
  if (name == "weight") {
...
print.xgb.DMatrix <- function(x, verbose = FALSE, ...) {
  ...
  #change
  if(length(getinfo(x, 'censor')) > 0) infos <- c(infos, 'censor')
